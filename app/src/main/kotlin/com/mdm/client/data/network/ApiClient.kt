...
class ApiClient {
     ...
     * retorna Failure con el mensaje de error del servidor. Si T es Unit, solo verifica success sin
     * intentar parsear data.
     */
            ...
            client.newCall(request).execute().use { response ->
                ...
                if (T::class == Unit::class) {
                    ...
                    return if (wrapper?.success == true)
                            @Suppress("UNCHECKED_CAST") MdmResult.Success(Unit as T)
                    else
                            MdmResult.Failure(
                                    wrapper?.error ?: wrapper?.message ?: "Error del servidor."
                            )
                }
                ...
                when {
                    wrapper == null -> MdmResult.Failure("No se pudo parsear la respuesta.")
                    !wrapper.success ->
                            MdmResult.Failure(
                                    wrapper.error ?: wrapper.message ?: "Error del servidor."
                            )
