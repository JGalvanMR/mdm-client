...
class UnlockDeviceHandler(private val context: Context) {
    ...
    fun execute(): ExecutionResult {
        if (!checker.isDeviceOwner()) {
            return ExecutionResult.failure("La app no es Device Owner. No se puede resetear el password.")
        }
        ...
        return try {
            ...
            ExecutionResult.success("""{"unlocked":true,"passwordRemoved":true,"timestamp":"${System.currentTimeMillis()}"}""")
        } catch (e: Exception) {
