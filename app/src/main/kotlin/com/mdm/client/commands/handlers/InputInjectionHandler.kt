...
class InputInjectionHandler(private val context: Context) {
    ...
    data class InputEvent(val type: String, val x: Float?, val y: Float?, val keyCode: Int?)

    fun processInput(jsonParams: String?): ExecutionResult {
        if (instance == null) {
            return ExecutionResult.failure(
                    "AccessibilityService no disponible. Activa el servicio MDM Screenshot en ajustes."
            )
        }
